	...
	if (ValidAlgorithmId(id)) {
		switch(options.type) {
			...
			case algorithm_type::boost_brent: {
				algorithms_map[id] = unique_ptr<BoostBrent>(new BoostBrent(options.options));
				break;
			}
			default:
				return 0;
...
void trainAlgorithmThread(uv_work_t* req) {
	...
	try {
		...
		if (BrentSearch* alg = dynamic_cast<BrentSearch*>(job->algorithm)) {
			...
			alg->finishRun();
		} else if (BoostBrent* alg = dynamic_cast<BoostBrent*>(job->algorithm)) {
			alg->setUpRun();
			if (FaSTLMM* model = dynamic_cast<FaSTLMM*>(job->model)) {
				alg->run(model);
			} else {
				throw runtime_error("Requested model type not implemented for the requested algorithm");
			}
...
MatrixXf Scheduler::getJobResult(const job_id_t job_id) {
	if (JobIdUsed(job_id)) {
		...
		if (AdaMultiLasso* model = dynamic_cast<AdaMultiLasso*>(job->model)) {
			...
			return model->getBeta();
		} else if (FaSTLMM* model = dynamic_cast<FaSTLMM*>(job->model)) {
			return model->getBeta();
		} else {
	    	return model->getBeta();
	    }
