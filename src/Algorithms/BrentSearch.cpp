...
void BrentSearch::set_a(float a) {
    this->a = a;
}

void BrentSearch::set_b(float b) {
    this->b = b;
}

void BrentSearch::set_c(float c) {
    this->c = c;
}

void BrentSearch::set_m(float m) {
    this->m = m;
}

void BrentSearch::set_e(float e) {
    this->e = e;
}

void BrentSearch::set_t(float t) {
    this->t = e;
}
...
void BrentSearch::set_min_cost_val(float min_cost) {
    this->min_val = min_cost;
}

// Stored the best parameter which gives the least cost func. val.
void BrentSearch::set_best_param_val(float best_val) {
    this->min_val_param = best_val;
}

float BrentSearch::get_a() {
    return this->a;
}

float BrentSearch::get_b() {
    return this->b;
}

float BrentSearch::get_c() {
    return this->c;
}

float BrentSearch::get_m() {
    return this->m;
}

float BrentSearch::get_e() {
    return this->e;
}

float BrentSearch::get_t() {
    return this->t;
}

float BrentSearch::get_delta() {
    return this->delta;
}

float BrentSearch::get_min_cost_val() {
    return this->min_val;
}

// Stored the best parameter which gives the least cost func. val.
float BrentSearch::get_best_param_val() {
    return this->min_val_param;
}

// Get all the basic brent parameteres in vector form
vector<float> BrentSearch::get_brent_params() {
...
void BrentSearch::run(shared_ptr<Model> model) {
    if(!model.get()) {
        return;
 ...
 */
void BrentSearch::sub_run(shared_ptr<LinearMixedModel> model) {
    mtx.lock();
    unique_ptr<GridSearch> gridSearch(new GridSearch());
    gridSearch->set_lambda_params(0.0,0.1,0.001);
    gridSearch->sub_run(model);

    float a0, a2, a3, d0, d1, d2, h, m2, macheps, p, q, qs, r, s, sc, y, y0, y1, y2, y3, yb, z0, z1, z2;
    float a, b, c, m, e, t, x = 0.0, best_grid_search_val = 0.0;
    int k;
    vector<float> params = this->get_brent_params();
